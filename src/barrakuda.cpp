...
int p_e = 3; // Número de padres élite
int p_t = 1; // Número de padres no élite
...
void read_parameters(int argc, char **argv) {
    ...
    while (iarg < argc) {
        ...
        else if (strcmp(argv[iarg],"-ls_iter")==0) ls_iter = atoi(argv[++iarg]);
...
void show_sub_instance(sub_instance_LRS sub_instance) {
    cout << "Sub-instance: ";
    for (int i = 0; i < int(runs.size()); ++i) {
        cout << sub_instance.runs[i];
    }
    cout << endl;
}
...
vector<int> run_cplex(clock_t& start, vector<double>& results, vector<double>& times, vector<double>& gaps, int& na,sub_instance_LRS sub_instance) {
...
int main( int argc, char **argv ) {
    ...
    for (int na = 0; na < n_files; ++na) {
        ...
        for (int pi = 0; pi < population_size; ++pi) {
            ...
            local_search(population[pi],generator,ls_iter);
        ...
        while (ctime < computation_time_limit) {
            ...
            for(int i=0 ; i<n_elites ;i++){
                local_search(population[i],generator,ls_iter);
                if(population[i].score>best_LCMSA_quality){
                    best_LCMSA_quality=population[i].score;
                    clock_t current10 = clock();
                    ctime = double(current10 - start) / CLOCKS_PER_SEC;
                    best_BRKGA_fitness = best_LCMSA_quality;
                    if (output_type==0) cout << "-BK- value " << best_BRKGA_fitness << "\ttime " << ctime << " ls " <<endl;
                }
            }*/
            ...
            //Solve phase
            //show_sub_instance(sub_instance);
            current_CPLEX_individual=run_cplex(start, results, times, gaps, na, sub_instance);
            ...
            */
            ...
            for(int i=0; i<number_CPLEX_solution_copies_injected; i++) {
                ...
                //local_search(population[random_position],generator,ls_iter);

            }
            
            /*
            for (int i = 0; i < number_CPLEX_solution_copies_injected; i++) {
                int worst_idx = population_size - 1 - i; // Reemplaza desde el peor
                for (int j = 0; j < runs.size(); j++) {
                    population[worst_idx].vec[j] = (current_CPLEX_individual[j] == 1) ? 0.75 : 0.25;
                }
                evaluate(population[worst_idx]);
            }*/
